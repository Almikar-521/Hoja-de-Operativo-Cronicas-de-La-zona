...
export const GeneralStore: React.FC<Props> = ({ char, onBuyItem, onClose }) => {
  ...
  const renderItemCard = (item: any, type: 'Item' | 'Weapon' | 'Armor') => {
    ...
    return (
      <div key={item.id} className={`border p-3 rounded flex flex-col justify-between transition-all shadow-sm ${
          isLegendary 
          ...
          <div className="flex gap-1 mt-2 flex-wrap">
            {item.rarity && (
                <span className={`text-[9px] px-1.5 py-0.5 rounded uppercase border ${
                    item.rarity === 'Civil' ? 'border-green-800 text-green-500 bg-green-900/10' :
        ...
        <button
          ...
          disabled={!isGmMode && !canAfford && price > 0}
          className={`mt-3 w-full py-1 text-xs font-bold uppercase rounded transition-colors ${
             isGmMode 
        ...
        <div className="flex border-b border-gray-700 bg-gray-900 flex-none overflow-x-auto scrollbar-hide">
             {(['Armamento', 'Blindaje', 'Equipo Táctico', 'Artefactos', 'Suministros', 'Vehículos'] as MainTab[]).map(tab => (
                <button 
                  ...
                  className={`py-4 px-6 text-sm font-bold uppercase transition-all border-b-2 whitespace-nowrap ${
                      activeTab === tab 
