...
export const useCharacter = () => {
            ...
            if (existingIdx >= 0) {
                ...
                if (qty < 0) {
                    ...
                    if (totalEquipped > nextQty) {
                        // FIX: Use loop to remove enough items until totalEquipped <= nextQty
                        let currentlyEquipped = totalEquipped;

                        while (currentlyEquipped > nextQty) {
                            // Try to remove from Artifacts first
                            ...
                            // Safety break to prevent infinite loops if state is inconsistent
                            break;
                        }
                    }
                }
    ...
    const handleLoadChar = (loadedChar: CharacterState) => {
        setChar({ ...INITIAL_CHAR, ...loadedChar });
    };
